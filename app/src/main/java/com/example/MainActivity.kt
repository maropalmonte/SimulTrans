...
class MainActivity : AppCompatActivity() {
    ...
    // Estado de grabación de audio (MediaRecorder), compartido por las 2 pestañas.
    ...
    /**
     * Inicia una grabación de audio con MediaRecorder hacia un archivo
     * temporal propio de la app. Se usa tanto desde el Traductor como
     * desde el Asistente IA; solo puede haber una grabación activa a la
     * vez (comparten el mismo MediaRecorder).
     */
    private fun iniciarGrabacion(): Boolean {
        return try {
            val archivo = File(cacheDir, "grabacion_voz.m4a")
            ...
            archivoGrabacionActual = archivo
            true
        } catch (e: Exception) {
    ...
    /** Detiene la grabación en curso y devuelve el archivo de audio resultante. */
    private fun detenerGrabacionYObtenerArchivo(): File? {
